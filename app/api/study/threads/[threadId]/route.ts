...
export async function GET(_: Request, context: Params) {
  ...
  const requestMeta = getRequestMeta({
    requestId,
    route: "/api/study/threads/[threadId]",
    method: "GET"
  });
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) {
    logEvent("warn", "study_thread.unauthorized", requestMeta);
    return NextResponse.json({ error: "Authentication required." }, { status: 401 });
  }

  const { threadId } = await context.params;
  const detail = await getStudyThreadDetail({
    userId: session.user.id,
    threadId
  });

  if (!detail) {
    logEvent("warn", "study_thread.not_found", {
      ...requestMeta,
      userId: session.user.id
    });
  ...
  logEvent("info", "study_thread.get_ok", { ...requestMeta, userId: session.user.id });
...
export async function PATCH(req: Request, context: Params) {
  ...
  try {
    ...
    const thread = await prisma.studyThread.update({
      where: { id: threadId },
      data: {
        archivedAt:
    ...
    logEvent("error", "study_thread.patch_failure", { ...requestMeta, error });
